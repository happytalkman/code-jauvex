...
// {args, key: whether TYPESAFE_API_KEY was given, cwd} (never the key). Words in the goal: BLOCK (it gets stuck), CRASH (the runner fails).
import { appendFileSync } from 'node:fs';
const args = process.argv.slice(2); const flag = (f: string) => { const i = args.indexOf(f); return i >= 0 ? args[i + 1] : undefined; };
if (process.env.MOCK_UV_LOG) appendFileSync(process.env.MOCK_UV_LOG, `${JSON.stringify({ args, key: !!process.env.TYPESAFE_API_KEY, cwd: process.cwd() })}\n`);
