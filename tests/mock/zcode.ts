...
// [[fail]] fails the turn. Sessions are kept in <ZCODE_DATA_BASE_DIR or ~>/.zcode/mock-sessions/<id>.json. No network, no model.
...
async function runTurn(s, text) {
  const sessionId = s.session.sessionId; const turnId = randomUUID(); const state = { cut: false }; running.set(sessionId, state); const started = Date.now();
  ...
  event(sessionId, turnId, 'turn.started', { turnNumber: s.messages.length, input: text, messageId: userId });
  ...
  const aid = randomUUID(); const answer = { info: { messageId: aid, sessionId, role: 'assistant', time: { created: Date.now() }, parentMessageId: userId, agent: 'build', path: { cwd: s.session.workspace.workspacePath, root: s.session.workspace.workspacePath }, cost: 0, tokens: { input: 0, output: 0, reasoning: 0, cache: { read: 0, write: 0 } } }, parts: [] as any[] };
  ...
  if (text.includes('[[tool]]')) {
    ...
    answer.parts.push(part); event(sessionId, turnId, 'part.upserted', { part });
  }
  const part = { type: 'text', partId: randomUUID(), sessionId, messageId: aid, text: '' };
  ...
  part.text = part.text.trim(); answer.parts.push(part); event(sessionId, turnId, 'part.upserted', { part });
