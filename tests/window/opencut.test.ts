import { connect, sleep, check, done } from './lib.ts'; import { execFileSync } from 'node:child_process'; import http from 'node:http'; import { mkdirSync } from 'node:fs';
...
const run = (...args: string[]) => { try { return { code: 0, out: JSON.parse(execFileSync('node', ['scripts/jauvex.ts', ...args], { env: { ...process.env }, encoding: 'utf8' })) }; } catch (e) { const x = e as { status: number; stdout: string }; return { code: x.status, out: (() => { try { return JSON.parse(x.stdout); } catch { return { raw: String(x.stdout) }; } })() }; } };
...
check('its address is set from the command line', run('settings', '--opencut-url', `127.0.0.1:${PORT}`).code === 0);
...
check('running: it opens in the pane', (await js("document.querySelector('.pane-body webview')?.getAttribute('src')")) === url);
...
check('the opencut command opens it too', run('opencut').code === 0 && (await sleep(800), (await js("document.querySelector('.pane-body webview')?.getAttribute('src')")) === url));
mkdirSync('tmp/opencut-folder', { recursive: true });
const f = run('settings', '--opencut-folder', 'tmp/opencut-folder'); await sleep(800);
...
check('a javascript: address is refused', run('settings', '--opencut-url', 'javascript:alert(1)').code === 1);
