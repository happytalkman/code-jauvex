...
const { stdinText } = await import('../shared/cli-json.ts');
check('stdin with a byte-order mark, or as UTF-16 (Windows PowerShell), is read as text', stdinText(Buffer.from('\uFEFF{"a":1}', 'utf8')) === '{"a":1}' && stdinText(Buffer.concat([Buffer.from([0xff, 0xfe]), Buffer.from('{"a":1}', 'utf16le')])) === '{"a":1}' && stdinText(Buffer.from('{"a":1}', 'utf16le')) === '{"a":1}');
check('a JSON argument that lost its quotes (Windows PowerShell) is said as such; piped in with -, it is read', !lost.ok && lost.error === QUOTES_LOST && piped.ok && (piped.value as { title: string }).title === 'x' && !jsonArg('nope').ok && /^not JSON \(it starts: "nope"\)$/.test((jsonArg('nope') as { error: string }).error));
