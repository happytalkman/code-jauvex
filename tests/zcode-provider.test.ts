...
// a message said during a turn is not taken (the window queues it), a turn can be stopped, a failed turn ends red, and the folder lists
// the session with the others. Pure parts first: how a ZCode message part becomes a message of the thread.
...
// a message said during a turn is not taken; the turn can be stopped
const t3 = start(sid, 'take your time [[slow]]'); let steered: boolean | null = null; let stopped = false;
for (let i = 0; i < 100 && !chat.isRunning(t3.chatId); i++) await new Promise((r) => setTimeout(r, 10));
await new Promise((r) => setTimeout(r, 80)); steered = await chat.steerChat(t3.chatId, 'and one more thing'); stopped = await chat.stopChat(t3.chatId);
const e3 = await t3.done;
check('a message during a ZCode turn is not taken (the window queues it)', steered === false);
check('a ZCode turn can be stopped', stopped && e3.at(-1)?.type === 'done' && !chat.isRunning(t3.chatId), JSON.stringify(e3.at(-1)));
