...
COMMAND: what the user just said, which may be an order for the app itself rather than for the main assistant. Answer with exactly one line and nothing else. NONE when it is for the main assistant: a coding request or a question, even if it mentions agents, or anything else. NEW <claude|codex|zcode|claw|jev|any> <folder|here> when they want a new agent, session or chat opened: the kind as they named it (speech-to-text writes Claude as \"Cloud\", Codex as \"codecs\", ZCode as \"Z code\" or \"zed code\", Jev as \"Jeff\", \"Jet\" or \"Jab\"; \"any\" when none was named: never guess a kind, \"a new agent\" or \"a new chat\" alone is \"any\"), then the folder they named from the FOLDERS line, or \"here\" for the open one or none named. RESTART when they want this app restarted or relaunched and nothing else (not a server, not a service in their code; a restart asked together with other work, like \"publish and restart the app\", is NONE: the assistant does the work, then restarts the app itself). Examples: \"make a new codecs agent in the homepage project\" -> \"NEW codex homepage\"; \"create a new agent class in the orchestrator\" -> \"NONE\"; \"restart the app please\" -> \"RESTART\".
...
function ask(message: string, provider: Provider, model: string, timeoutMs: number): Promise<string> {
  if (provider === 'claw') { const t0 = Date.now(); const kind = /^(HEARD|BUSY|DONE|COMMAND):/m.exec(message)?.[1] ?? 'OTHER'; return claw.voiceAsk(VOICE_PROMPT, message, model, timeoutMs + 1500).then((t) => { debug.log('model', `voice model (Claw ${claw.voiceModel(model)}) ${kind}: ${t.trim() || '(nothing)'}`, { by: 'voice model', ms: Date.now() - t0 }); return t; }, (e: Error) => { debug.log('note', `voice model (Claw) failed: ${e.message}`, { by: 'voice model' }); return ''; }); } // one claw per question: slow to start, so its lines come late
...
export function warmAck(provider: Provider, model: string): void {
  ...
  if (provider === 'claw') return; // nothing to warm: every question is its own claw
...
type Details = { kind?: 'claude' | 'codex' | 'zcode' | 'claw' | 'jev' | null; name?: string | null; purpose?: string | null; folder?: string | null; kickoff?: string | null };
async function oneShot(provider: Provider, model: string | undefined, prompt: string, timeoutMs: number): Promise<string> {
  const run = async (): Promise<string> => {
    ...
    if (provider === 'claw') return claw.runOnce(prompt, model, timeoutMs);
    let text = '';
...
export async function command(full: string, projects: { id: string; name: string; path?: string }[], currentId: string, speaker?: { provider: Provider; model: string; main: string; mainModel?: string }): Promise<AppCommand | null> {
  ...
  if (!r || !r.answers.target || r.answers.target.confidence < JEV_MIN_CONFIDENCE) {
    if (speaker) {
      ...
      const m = /^\W*(NONE|NEW|RESTART)\b\s*(claude|codex|zcode|claw|jev|any)?\s*(.*)$/i.exec(out.trim());
      if (m) {
...
export async function status(): Promise<VoiceStatus> { return { jev: await jev.available(), jevKey: await jev.hasKey(), whisper: whisperState, detail: whisperDetail, voices: await voices(), models: listModels(), model: whisperDetail.endsWith('.bin') ? whisperDetail : '', voiceModels: { claude: (claudeModelCache ?? []).map(({ id, label, resolved }) => ({ id, label, ...(resolved ? { resolved } : {}) })), codex: (await codex.models().catch(() => [])).map(({ id, label }) => ({ id, label })), zcode: (await zcode.models().catch(() => [])).map(({ id, label }) => ({ id, label })), claw: claw.CLAW_MODELS.map(({ id, label }) => ({ id, label })) } }; }
/** The model the voice would use now for a provider and a preference (for the settings' "in use" line). */
export async function voiceModelInUse(provider: Provider, preferred: string): Promise<string> { if (provider === 'codex') return codex.voiceModel(preferred).catch(() => preferred); if (provider === 'zcode') return zcode.voiceModel(preferred); if (provider === 'claw') return claw.voiceModel(preferred); await claudeModels().catch(() => undefined); return resolveVoiceModel('claude', preferred); }
