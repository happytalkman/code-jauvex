...
export function Welcome({ onDone, defaultProvider, onDefault, jauvexMove, onJauvexMove }: { onDone: (start?: boolean) => void; defaultProvider: Provider | null; onDefault: (p: Provider) => void; jauvexMove: 'unified' | 'handoff'; onJauvexMove: (m: 'unified' | 'handoff') => void }) {
  const [rows, setRows] = useState<Row[]>([
    ...
    { id: 'claw', label: 'Claw', state: 'wait', detail: 'Looking for the claw command and an API key', say: '' },
  ...
  useEffect(() => {
    ...
    const signedInNow = (provider: Provider) => void window.desktop.accountStatus(provider).then((st) => { if (stopped.current) return; if (!st.signedIn) { set(provider, { state: provider === 'zcode' || provider === 'claw' ? 'opt' : 'no', detail: provider === 'zcode' || provider === 'claw' ? `${NAME[provider]} is still not ready${st.error ? `: ${st.error}` : '.'}` : `${NAME[provider]} is still not signed in. Sign in with ${SIGN_IN_CLI[provider].tool}, in Terminal, then check again.` }); return; } const e = { provider }; set(e.provider, { state: 'ok', detail: e.provider === 'zcode' ? `ZCode is ready${st.who ? `, on ${st.who}` : ''}.` : e.provider === 'claw' ? `Claw is ready (${st.method || 'claw'}, on an Anthropic API key).` : `${NAME[e.provider]} is signed in${st.who ? ` as ${st.who}` : ''}.`, action: undefined }); setSigned((x) => (x.includes(e.provider) ? x : [...x, e.provider])); if (!chosenRef.current) { setChosen(e.provider); onDefaultRef.current(e.provider); } canAnswer.current = true; readyRef.current = rows.find((r) => r.id === 'whisper')?.state !== 'no'; void speak(`${NAME[e.provider]} is ${e.provider === 'zcode' ? 'ready' : 'signed in'}. Say start, or press the button.`); });
    ...
    void (async () => {
      ...
      const [setup, claude, codex, zcode, claw] = await Promise.all([window.desktop.setupCheck().catch(() => null), window.desktop.accountStatus('claude').catch(() => null), window.desktop.accountStatus('codex').catch(() => null), window.desktop.accountStatus('zcode').catch(() => null), window.desktop.accountStatus('claw').catch(() => null)]) as [SetupCheck | null, AccountStatus | null, AccountStatus | null, AccountStatus | null, AccountStatus | null];
      ...
      const win = setup?.os === 'win32'; // the web version on Windows: its own speech, whisper from npm run voice:setup
      if (win) set('say', { label: 'Speech on this computer' });
      await land('say', win ? { state: 'ok', detail: "Windows' own speech is here, so I can talk." } : setup?.say ? { state: 'ok', detail: 'The say command is here, so I can talk.' } : { state: 'no', detail: 'No say command: speech needs macOS, or Windows in the web version.', hint: 'On Windows: npm run web' });
      await land('voice', setup?.voice === 'system' ? { state: 'ok', detail: "Windows' default voice speaks. To change it: Settings > Time & language > Speech." } : setup?.voice === 'basic' ? { state: 'opt', detail: 'The System voice is the basic Samantha, which sounds robotic. Pick a Siri voice: System Settings > Accessibility > Spoken Content > System voice. An app cannot choose it for you.', action: { label: 'Open Spoken Content', url: SPOKEN_CONTENT } } : setup?.voice === 'natural' ? { state: 'ok', detail: 'A natural voice is set as the System voice.' } : { state: 'opt', detail: 'Could not tell which voice is set. If it sounds robotic, pick a Siri voice in Spoken Content.', action: { label: 'Open Spoken Content', url: SPOKEN_CONTENT } });
      ...
      // Claw is optional too: ready once its command answers and its doctor sees an Anthropic API key (it has no sign-in of its own).
      await land('claw', claw?.signedIn ? { state: 'ok', detail: `Claw is ready (${claw.method || 'claw'}, on an Anthropic API key).` } : { state: 'opt', detail: `${claw?.error ?? 'Claw is not here.'} Optional.`, hint: claw?.error?.includes('not found') ? SIGN_IN_CLI.claw.install : SIGN_IN_CLI.claw.login, action: { label: 'Check again', run: () => signedInNow('claw') } });
      ...
      const wh = !setup?.whisperBinary ? { state: 'no' as const, detail: 'whisper-server is not installed, so I cannot hear you yet.', hint: win ? 'npm run voice:setup (whisper-server and the models, into this folder)' : 'brew install whisper-cpp, then download a model into the models folder (see the README)' } : !setup.models.length ? { state: 'no' as const, detail: 'whisper-server is here, but there is no model to listen with.', hint: win ? 'npm run voice:setup' : 'Download a model into the models folder (see the README)' } : up?.whisper === 'ready' ? { state: 'ok' as const, detail: `Whisper is ready with ${setup.models.length} model${setup.models.length > 1 ? 's' : ''}.` } : { state: 'no' as const, detail: `I cannot hear you yet: ${up?.detail || 'Whisper did not answer within a minute'}.` };
      ...
      const list: Provider[] = [...(claude?.signedIn ? ['claude' as const] : []), ...(codex?.signedIn ? ['codex' as const] : []), ...(zcode?.signedIn ? ['zcode' as const] : []), ...(claw?.signedIn ? ['claw' as const] : [])];
