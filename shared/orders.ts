...
const AGENT_WORDS: ['zcode' | 'codex' | 'jev' | 'claude' | 'claw', RegExp][] = [['claw', /\b(claw|claws|clawcode|claw ?code)\b/i], ['zcode', /\b(z ?-?code|zed ?code|zee ?code|zhipu|glm)\b/i], ['codex', /\b(codex|codecs|code ?x|kodex|chat ?gpt|open ?ai|gpt)\b/i], ['jev', /\b(jev|jeff|jet|jab|jav|jeb|classifier)\b/i], ['claude', /\b(claude|cloud|clod|claud|clawed|anthropic)\b/i]];
