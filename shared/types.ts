...
export const GRAPH_NOTE = 'A graph database for your use. WEAIDdb (a HydraDB graph database, queried with OpenCypher) can run beside this app: keep what is worth remembering as nodes and relationships, and query it. Claude sessions have the graph_query tool; from any session, run `node scripts/graph.ts "<cypher>"`, with values as $parameters (`--params \'{"name":"x"}\'`) rather than pasted into the query; `node scripts/graph.ts status` says whether it is running. Every agent shares it: give each node a `project` property naming the folder it belongs to, and read before you write, so nothing is duplicated.';
