...
export const BROWSE_NOTE = 'A browser agent for your use. When a task needs a real web page (search a site, fill a form, find something there), hand it to the browser agent: it drives the user\'s Chrome toward one goal on its own, then says how it ended, each step, and what was on the last page. Claude sessions have the browse tool; from any session, run `node scripts/browse.ts --url <https://...> --goal "<what to do, and when to stop>"`. It takes a minute or two and uses the user\'s TypeSafe key and text model: one narrow goal per run, and check the last page it reports before you say the task is done (its DONE is a choice, not a proof).';
