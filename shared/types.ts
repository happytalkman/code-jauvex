...
export type Provider = 'claude' | 'codex' | 'zcode' | 'claw';
export const PROVIDERS: Provider[] = ['claude', 'codex', 'zcode', 'claw'];
/** Claw's models: claw's own aliases (its default, Opus, is left to a choice; electron/claw.ts). */
export const CLAW_MODELS: ModelOption[] = [{ id: 'sonnet', label: 'Sonnet', isDefault: true }, { id: 'opus', label: 'Opus' }, { id: 'haiku', label: 'Haiku' }];
export const PROVIDER_LABEL: Record<Provider, string> = { claude: 'Claude', codex: 'Codex', zcode: 'ZCode', claw: 'Claw' };
...
export type UiState = { sel?: { projectId: string; sessionId: string } | null; sidebar?: boolean; showMeta?: boolean; voice?: Partial<VoiceSettings>; welcomed?: boolean; defaultProvider?: Provider; jauvexSession?: string | null; jauvexProvider?: Provider; jauvexMove?: 'unified' | 'handoff'; showJauvex?: boolean; opencut?: { url?: string; folder?: string } /* OpenCut beside the app: its address and its folder (shared/opencut.ts) */; autoCompact?: number /* compact an agent's conversation when its context is this full, in percent; 0: the provider decides (shared/context.ts) */ };
...
export const APP_WORDS = ['Jauvex', 'Jev', 'Claude', 'Codex', 'ZCode', 'Claw'];
...
export type AgentCommand =
  ...
  | { type: 'settings'; defaultProvider?: Provider; showJauvex?: boolean; welcomeNext?: boolean; jauvexMove?: 'unified' | 'handoff'; autoCompact?: number; opencutUrl?: string; opencutFolder?: string }
  | { type: 'welcome' } | { type: 'opencut' } | { type: 'reload-ui' } | { type: 'restart-app' };
...
export const SIGN_IN_CLI: Record<Provider, { login: string; logout: string; tool: string; install: string }> = {
  ...
  claw: { login: 'setx ANTHROPIC_API_KEY sk-ant-... (Windows; export ANTHROPIC_API_KEY=... on a Mac), then start the app again', logout: 'remove ANTHROPIC_API_KEY', tool: 'Claw', install: 'cargo build --release in a clone of github.com/ultraworkers/claw-code (rust/), then put its claw on the PATH; on Windows the setup does it: scripts/windows-start.ps1' },
  zcode: { login: 'zcode login', logout: 'zcode logout', tool: 'ZCode', install: 'pnpm build:zcode (in a clone of github.com/zai-org/ZCode), then put its zcode on the PATH; on Windows the setup does it: scripts/windows-start.ps1' },
};
export const AGENT_COMMANDS_HELP = 'node scripts/jauvex.ts <command> [--flag value ...], from any folder. Commands: list (folders, sessions and agents, with ids); add-folder <path>; pick-folder (opens the folder dialog for the user, adds what they choose); new-agent [--provider claude|codex|zcode|claw|jev] [--folder <name|path|id>] [--name "..."] [--purpose "..."] [--kickoff "first message"] (the agent starts at once, with its own introduction when no first message is given: it exists, and is listed, from then on); open [--folder ...] [--session <id|title>] (no session: the Jauvex agent); send --session <id|title> [--folder ...] --text "..."; rename --session <id|title> --title "..."; settings [--default-provider claude|codex|zcode|claw] [--show-jauvex yes|no] [--welcome-next yes|no] [--jauvex-move unified|handoff] [--auto-compact <percent>|provider] (compact an agent\'s conversation when its context is that full) [--opencut-url <address>] [--opencut-folder <path>] (OpenCut, the video editor, run on its own: where it answers, and its folder, which is also added to the sidebar); opencut (opens OpenCut in the right pane); welcome; reload (the window, after a UI build); restart (the app, after a main-process build; it kills your own turn: last thing you do). Each prints a JSON result.';
...
/** WEAIDdb for every agent (shared/graph.ts): how to reach it, and to keep it tidy, since every agent shares it. */
export const GRAPH_NOTE = 'A graph database for your use. WEAIDdb (a HydraDB graph database) can run beside this app: keep what is worth remembering across sessions as nodes and relationships, and query it. Claude sessions have the graph_query tool; from any session, run `node scripts/graph.ts "<cypher>" --params \'{"k":"v"}\'` (`node scripts/graph.ts status` says whether it is running). It speaks a subset of OpenCypher, one statement at a time: every node has an integer `id` you choose (`node scripts/graph.ts newid` prints a fresh one); a write is a CREATE or MERGE of a relationship path, e.g. `MERGE (p:Project {id: $pid, name: $name})-[:HAS_NOTE]->(n:Note {id: $nid, text: $text})`; a change is `MATCH (n:Note {id: $nid}) SET n.text = $text`; a read is `MATCH (p:Project {id: $pid})-[:HAS_NOTE]->(n:Note) RETURN n.id AS id, n.text AS text`, with WHERE comparisons (=, <>, <, >, STARTS WITH; not CONTAINS or IN) and count(*) (no max or min). Values go as $parameters, never pasted into the query (in Windows PowerShell pipe the JSON in and pass `--params -`: it drops the quotes of an argument). Every agent shares it: one Project node per folder (find it first: `MATCH (p:Project) WHERE p.name = $name RETURN p.id AS id`), and read before you write so nothing is duplicated.';
/** jev-ultrafast for every agent (shared/browse.ts). */
export const BROWSE_NOTE = 'A browser agent for your use. When a task needs a real web page (search a site, fill a form, find something there), hand it to the browser agent: it drives a Chrome window of its own (its own profile: the user\'s sign-ins are not there) toward one goal on its own, then says how it ended, each step, and what was on the last page. Claude sessions have the browse tool; from any session, run `node scripts/browse.ts --url <https://...> --goal "<what to do, and when to stop>"`. It takes a minute or two and uses the user\'s TypeSafe key and text model: one narrow goal per run, and check the last page it reports before you say the task is done (its DONE is a choice, not a proof).';
/** Paperclip for every agent (shared/paperclip.ts). */
export const PAPERCLIP_NOTE = 'Paperclip, where the user\'s goals and tasks live. Paperclip (an app that runs teams of AI agents like a company: goals, projects, issues, comments, approvals, budgets) can run beside this app, and this app can be connected to one of its companies as an agent named Jauvex. When the user speaks of goals, tasks, issues or the board, look there. Reading and opening issues: Claude sessions have its tools (paperclipListIssues, paperclipGetIssue, paperclipCreateIssue and the rest), and from any session `node scripts/paperclip.ts GET /api/companies/{companyId}/issues` ({companyId} is filled in; `node scripts/paperclip.ts status` says whether it runs). Comments and status changes: always `node scripts/paperclip.ts POST /api/issues/<id>/comments \'{"body":"..."}\'` and `node scripts/paperclip.ts PATCH /api/issues/<id> \'{"status":"in_progress"}\'` (statuses: backlog, todo, in_progress, in_review, done; in Windows PowerShell, which drops the quotes of an argument, pipe the JSON in and pass - instead: `\'{"body":"..."}\' | node scripts/paperclip.ts POST /api/issues/<id>/comments -`); Paperclip lets an agent write those only inside a run of its own, so this command sends them as the user, marked [from Jauvex]. Report what you did on the issue you worked on, as a comment.';
...
].join('\n\n').replaceAll('{APP}', appRoot ?? 'the app\'s install folder').replaceAll('node scripts/jauvex.ts', appRoot ? `node "${appRoot}/scripts/jauvex.ts"` : 'node scripts/jauvex.ts').replaceAll('node scripts/graph.ts', appRoot ? `node "${appRoot}/scripts/graph.ts"` : 'node scripts/graph.ts').replaceAll('node scripts/browse.ts', appRoot ? `node "${appRoot}/scripts/browse.ts"` : 'node scripts/browse.ts').replaceAll('node scripts/paperclip.ts', appRoot ? `node "${appRoot}/scripts/paperclip.ts"` : 'node scripts/paperclip.ts');
