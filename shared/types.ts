...
].join('\n\n').replaceAll('{APP}', appRoot ?? 'the app\'s install folder').replaceAll('node scripts/jauvex.ts', appRoot ? `node "${appRoot}/scripts/jauvex.ts"` : 'node scripts/jauvex.ts').replaceAll('node scripts/graph.ts', appRoot ? `node "${appRoot}/scripts/graph.ts"` : 'node scripts/graph.ts');
